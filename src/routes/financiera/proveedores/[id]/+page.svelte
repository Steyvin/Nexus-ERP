<script lang="ts">
	...
	const proveedor = $derived(data.proveedor)
	...
	$effect(() => {
		if (!editando) {
			editNombre = proveedor.nombre
			editContacto = proveedor.contacto ?? ''
			editEmail = proveedor.email ?? ''
			editCiudad = proveedor.ciudad ?? ''
			editNotas = proveedor.notas ?? ''
			editActivo = proveedor.activo
		}
	})
...
<svelte:head>
	<title>{proveedor.nombre} — Nexus LED</title>
</svelte:head>
...
<div>
	...
	<div class="flex flex-wrap items-start justify-between gap-3">
		<div class="flex items-center gap-3">
			<a href="/financiera/proveedores" class="text-[var(--text-dim)] hover:text-[var(--text)]" aria-label="Volver">
			...
			<div>
				<div class="flex items-center gap-2">
					<h1 class="text-xl font-semibold text-[var(--text)]">{proveedor.nombre}</h1>
					{#if !proveedor.activo}
						<span class="rounded bg-[var(--bg-card-2)] px-1.5 py-0.5 text-[10px] text-[var(--text-dim)]">Inactivo</span>
					{/if}
				</div>
				<p class="mt-0.5 text-xs text-[var(--text-dim)]">{proveedor.contacto ?? proveedor.email ?? '—'}</p>
			</div>
		</div>
	...
	<div class="mt-5 grid gap-3 sm:grid-cols-3">
		<div class="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-5">
			...
			<p class="mt-1 text-2xl font-bold {Number(proveedor.deuda) > 0 ? 'text-red-400' : 'text-green-400'}">
				{fmt(proveedor.deuda)}
			</p>
		</div>
		<div class="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-5">
			<p class="text-xs text-[var(--text-muted)]">Total créditos</p>
			<p class="mt-1 text-2xl font-semibold text-[var(--text)]">{fmt(proveedor.total_creditos)}</p>
		...
		<div class="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-5">
			...
			<p class="mt-1 text-2xl font-semibold text-green-400">{fmt(proveedor.total_pagos)}</p>
	...
	{#if editando}
		<form method="POST" action="?/actualizar" use:enhance={() => {
			...
			<div class="grid gap-3 sm:grid-cols-2">
				...
				<div>
					<label class="mb-1 block text-xs font-medium text-[var(--text-muted)]">Email</label>
					<input name="email" type="email" bind:value={editEmail} class="input-field w-full" />
				</div>
				<div>
					<label class="mb-1 block text-xs font-medium text-[var(--text-muted)]">Ciudad</label>
					<input name="ciudad" type="text" bind:value={editCiudad} class="input-field w-full" />
				</div>
			</div>
			...
			<div class="mt-4 flex justify-between">
				{#if esAdmin}
					<form method="POST" action="?/eliminar" use:enhance={() => {
						...
						<button
							type="submit"
							onclick={(e) => { if (!confirm('¿Eliminar este proveedor? Solo puede eliminarse si no tiene créditos ni pagos.')) e.preventDefault() }}
	...
	<div class="mt-6 grid gap-4 lg:grid-cols-2">
		...
		<div class="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] overflow-hidden">
			<div class="border-b border-[var(--border)] px-5 py-3">
				...
				<p class="text-[10px] text-[var(--text-dim)]">Compras a crédito que aumentan la deuda</p>
			</div>
			...
			<form method="POST" action="?/agregarCredito" use:enhance={() => {
				...
				<div>
					...
					<input
						...
						required
						placeholder="Ej: Compra de acrílico, factura #123..."
			...
			{:else}
				<form method="POST" action="?/agregarPago" use:enhance={() => {
					...
					<div>
						...
						<input id="pago_concepto" name="concepto" type="text" bind:value={pagoConcepto} placeholder="Ej: Abono factura #123" class="input-field w-full" />
					</div>
